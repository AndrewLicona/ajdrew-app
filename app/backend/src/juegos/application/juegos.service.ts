...
export class JuegosService {
    ...
    async create(createJuegoDto: CreateJuegoDto) {
        ...
        if (existing) {
            throw new ConflictException(`Ya existe un juego con el slug "${createJuegoDto.slug}"`);
        }
    ...
    async findOne(idOrSlug: string) {
        ...
        const globalCategorias = await this.categoriasService.findAll(null as any, true);
        ...
        for (const cat of sortedCategories) {
            ...
            const rankedItems = await this.calificacionRepository.getRanking(cat.id, 6);

            // Transform back to simple item structure expected by frontend
            (cat as any).items = rankedItems.map(r => ({
                id: r.itemId,
                nombre: r.itemName,
                image: r.itemImage,
    ...
    async update(id: string, updateJuegoDto: UpdateJuegoDto) {
        if (updateJuegoDto.slug) {
            ...
            if (existing && existing.id !== id) {
                throw new ConflictException(`Ya existe otro juego con el slug "${updateJuegoDto.slug}"`);
            }
        }
