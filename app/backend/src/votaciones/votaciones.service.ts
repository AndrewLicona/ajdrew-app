...
import {
  BracketMatchAbiertoEvent,
  BracketMatchCerradoEvent,
} from '../modules/social-media/services/bracket-media.service';
...
export class VotacionesService {
    ...
    try {
      ...
      if (!allowedSizes.includes(dto.itemsIds.length)) {
        throw new BadRequestException(
          'El número de participantes debe ser exactamente 2, 4, 8 o 16.',
        );
      }
      ...
      if (
        data.estado === 'ACTIVA' &&
        data.rondaDuracion &&
        data.rondaDuracion > 0
      ) {
        (data as any).proximoCierreAt = this.repository.calculateNextDeadline(
          data.rondaDuracion,
        );
      }

      // Si inicia ACTIVA, emitir evento de inicio de fase para Ronda 1
      const created = await this.repository.create(data, matches);

      // Siempre notificar redes sociales de nuevo bracket creado
      this.eventEmitter.emit('social.bracket.created', {
      ...
      if (error.code === 'P2002') {
        throw new BadRequestException(
          'El slug ya está en uso. Por favor, elige otro título.',
        );
      }
      throw error;
    }
    ...
    if (deviceId) {
      ...
      const matchesWithVotes = await Promise.all(
        (bracket.matches || []).map(async (match: any) => {
          const vote = await (this.repository as any).getVote(
            match.id,
            deviceId,
          );
          return {
  ...
  async update(id: string, dto: any) {
    ...
    if (dto.itemsIds && dto.itemsIds.length > 0) {
      if (bracket.estado !== 'BORRADOR') {
        throw new BadRequestException(
          'No se pueden cambiar los participantes de un torneo en curso.',
        );
      }
    ...
    try {
      ...
      // Si el estado cambia de BORRADOR a ACTIVA
      if (bracket.estado === 'BORRADOR' && data.estado === 'ACTIVA') {
        this.eventEmitter.emit('bracket.phase.started', {
          bracketId: id,
          round: 1,
        });
  ...
  async advanceRound(id: string) {
    ...
    if (bracket.estado !== 'ACTIVA')
      throw new BadRequestException('Solo se pueden avanzar torneos activos');
    ...
    const matches = await this.repository.findMatchesByRoundWithItems(
      id,
      currentRound,
    );
    ...
    for (const match of matches) {
      ...
      this.eventEmitter.emit(
        'bracket.match.cerrado',
        new BracketMatchCerradoEvent(
          match.id,
          bracket.tematica,
    ...
    if (winnersIds.length <= 1) {
      // 🏆 Emitir evento de campeón antes de finalizar
      if (winnersIds.length === 1) {
        this.eventEmitter.emit('bracket.champion.declared', {
          bracketId: id,
    ...
    const nextDeadline =
      (bracket as any).rondaDuracion > 0
    ...
    for (const m of nextMatches) {
      ...
      const itemB = m.itemBId
        ? await (this.repository as any).findOneItem(m.itemBId)
        : null;

      this.eventEmitter.emit(
        'bracket.match.abierto',
        new BracketMatchAbiertoEvent(
    ...
    // Emitir evento para redes sociales de avance de fase
    this.eventEmitter.emit('bracket.phase.started', {
      bracketId: id,
      round: nextRound,
    });
  ...
  async vote(matchId: string, itemId: string, deviceId: string) {
    if (!deviceId)
      throw new BadRequestException('Se requiere deviceId para votar');
    ...
    if (!bracket)
      throw new BadRequestException('Bracket asociado no encontrado');

    if (bracket.estado !== 'ACTIVA') {
      throw new BadRequestException(
        `No se puede votar en un torneo ${bracket.estado.toLowerCase()}`,
      );
    }

    if (match.ronda !== bracket.rondaActual) {
      throw new BadRequestException(
        'Solo se permiten votos para la ronda actual en curso',
      );
    }
    ...
    if (!isA && !isB)
      throw new BadRequestException('El ítem no pertenece a este match');
