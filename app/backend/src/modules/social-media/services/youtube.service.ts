...
export class YoutubeService {
  ...
  async publishPhaseAnnouncement(
    ...
    await this.publishVideoFromImage(
      imageUrl,
      `Elite Rankings - Ronda ${round}`,
      `La Ronda ${round} ha comenzado. ¡Ven a votar!`,
      ['EliteRankings', 'Torneo'],
      { bracketId, round }
    );
  }

  async publishVideoFromImage(
    imageUrl: string,
    title: string,
    description: string,
    tags: string[] = [],
    metadata?: { bracketId?: string; round?: number; sorteoId?: string }
  ) {
    const id = metadata?.bracketId || metadata?.sorteoId || 'GENERIC';
    const round = metadata?.round || 0;
    ...
    const videoPath = await this.createVideoFromImage(
      imageUrl,
      id,
      round,
    );
    ...
    try {
      ...
      this.logger.error(
        `Error in YouTube publication process: ${error.message}`,
      );
  ...
  private async createVideoFromImage(
    ...
    return new Promise((resolve, reject) => {
      ...
      const inputPath = isLocalBracket
        ? imageUrl.replace(
            /.*\/brackets\//,
            path.join(process.cwd(), 'public', 'brackets', ''),
          )
        : imageUrl;

      this.logger.log(
        `Creating video from image. Input: ${inputPath}, Output: ${outputPath}`,
      );
  ...
  private async uploadToYoutube(
    ...
    try {
      ...
      const response = await youtube.videos.insert({
        ...
        requestBody: {
          snippet: {
            title,
            description,
            tags: [...tags, 'AJDREW', 'EliteRankings'],
      ...
      if (response.data.id) {
        this.logger.log(
          `YouTube Insert Response: ${JSON.stringify(response.data.status)}`,
        );
        ...
        this.logger.log(
          `Uploaded video to YouTube: https://youtu.be/${response.data.id} for account ${account.name}`,
        );
      ...
      this.logger.error(
        `YouTube upload failed for ${account.name}: ${error.message}`,
      );
