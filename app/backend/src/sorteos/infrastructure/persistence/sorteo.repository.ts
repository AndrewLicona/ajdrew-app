...
export class SorteoRepository {
      ...
      data: {
        ...
        ...(tareas !== undefined && {
          tareas: {
            deleteMany: {},
            create: tareas.map(
              ({ sorteoId, createdAt, updatedAt, ...t }: any) => t,
            ),
          },
        }),
      },
  ...
  async addParticipante(
    ...
    console.log('📝 SorteoRepository: Creating participant', {
      sorteoId,
      cleanData,
    });
  ...
  async completeTask(
    ...
    taskId: string,
    evidencia?: { url?: string; texto?: string },
  ) {
  ...
  async finalizeAndWinners(sorteoId: string, winners: any[]) {
    return this.prisma.$transaction([
      // Create winners
      ...winners.map((w) =>
        (this.prisma as any).sorteoWinner.create({
          data: {
            sorteoId,
